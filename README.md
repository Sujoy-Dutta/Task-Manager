...
TaskMind lets users sign up, log in, and manage a personal list of tasks. Each task has a title, description, due date, priority level, and a status that can be toggled between active and completed. All task data is scoped to the authenticated user and persisted in MongoDB.
...
| Layer | Technologies |
|-------|-------------|
| **Frontend** | React 19, React Router 7, Axios, Lucide React, Vite, TypeScript |
...
└── frontend/                 # React SPA (Vite)
    ...
    │   ├── utils/api.ts       # Axios instance with 401 handling
...
The frontend expects the backend to be available at `http://localhost:5000`. Update `src/utils/api.ts` if you run on a different port.
